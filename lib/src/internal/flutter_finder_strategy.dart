...
class FlutterFinderStrategy {
  ...
  static Future<FlutterElement> _locateElement(
      Map<String, dynamic> strategy) async {
    Finder? by;
    switch (strategy['finderType']) {
      ...
      case "ByText":
        ...
        break;
      case "Ancestor":
        by = await _byAncesstor(strategy);
        break;
    }
    bool elementExists = by != null ? !by.evaluate().isEmpty : false;
    if (!elementExists) {
      throw ElementNotFoundException(
          "Unable to locate element with strategy: ${strategy['finderType']}");
    }
    return FlutterElement(by!, Uuid().v4());
  }

  static Finder? _byValueKey(Map<String, dynamic> strategy) {
    if (strategy['keyValueString']) {
      return find.byKey(ValueKey(strategy['keyValueString']));
    }
    return null;
  }
  ...
  static Future<Finder?> _byAncesstor(Map<String, dynamic> strategy) async {
    if (strategy['of'] && strategy['matching']) {
      var of = await findElement(strategy["of"]);
      var matching = await findElement(strategy["matching"]);
